...
class LeashWebSocketClient(
    ...
    val activitiesPerAgent: StateFlow<Map<String, List<AgentActivity>>> =
            _activitiesPerAgent.asStateFlow()
    ...
    val chatMessagesPerAgent: StateFlow<Map<String, List<ChatMessage>>> =
            _chatMessagesPerAgent.asStateFlow()
    ...
    fun connect() {
        ...
        // Append password as query parameter if provided
        val urlWithAuth =
    ...
    fun sendInterrupt(agentId: String) {
        val json = gson.toJson(mapOf("type" to "interrupt", "agentId" to agentId))
        webSocket?.send(json)
    }

    fun subscribeToAgent(agentId: String) {
        val json = gson.toJson(mapOf("type" to "subscribe", "agentId" to agentId))
        webSocket?.send(json)
    }
    ...
    /**
     * Fetch chat history for an agent from the server. This fetches the full conversation
     * transcript.
     */
    suspend fun fetchChatHistory(agentId: String): List<ChatMessage> =
            withContext(Dispatchers.IO) {
                try {
                    val requestBuilder =
                            Request.Builder().url("$httpBaseUrl/api/agents/$agentId/chat").get()

                    // Add Authorization header if password is set
                    if (password.isNotEmpty()) {
                        requestBuilder.addHeader("Authorization", password)
                    }
    ...
    private fun handleActivity(json: JsonObject) {
        ...
        scope.launch { _activities.emit(activity) }
    }

    private fun handleStatusChange(json: JsonObject) {
        val agentId = json.get("agentId").asString
        val status = AgentStatus.fromString(json.get("status").asString)
        _agents.value = _agents.value.map { if (it.id == agentId) it.copy(status = status) else it }
    }
    ...
    private fun handleChatMessage(json: JsonObject) {
        ...
        scope.launch { _chatMessages.emit(Pair(agentId, chatMessage)) }
    }
    ...
    private fun handleMessageSent(json: JsonObject) {
        ...
        scope.launch { _messageSentStatus.emit(MessageSentStatus(agentId, success, hint)) }
    }

    private fun handleError(json: JsonObject) {
        val error = json.get("error")?.asString ?: "Unknown error"
        scope.launch { _messageSentStatus.emit(MessageSentStatus("", false, error)) }
    }
...
data class MessageSentStatus(val agentId: String, val success: Boolean, val hint: String)
